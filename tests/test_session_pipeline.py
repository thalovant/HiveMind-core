...
def _make_protocol(policy_protocols=None):
    ...
    db_user.account_id = "acct-server"
    db_user.hub_id = "hub-server"
    db_user.policy_group_id = "policy-server"
...
class TestSessionPipelineHandling(unittest.TestCase):
    ...
    def test_policy_context_uses_server_owned_account_fields(self):
        ...
        bus_message = _make_bus_message({
            ...
            "hub_id": "client-controlled",
            "policy_group_id": "client-controlled",
        })
        ...
        self.assertEqual(policy.contexts[-1].account_id, "acct-server")
        self.assertEqual(policy.contexts[-1].hub_id, "hub-server")
        self.assertEqual(policy.contexts[-1].policy_group_id, "policy-server")
